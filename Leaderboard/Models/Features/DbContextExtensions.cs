...
namespace Leaderboard.Models.Features
{
    public static class DbContextExtensions
    {
        ...
        /// <summary>
        /// Gets all DbSets of an entity type that implements IDbSeed<> and calls the seed method
        /// on configuring
        /// </summary>
        /// <param name="ctx"></param>
        /// <param name="builder"></param>
        public static void SeedData(this DbContext ctx, ModelBuilder builder)
            => ForEachDbSetEntity(ctx, configuringType =>
        {
            // IDbSeed<> has a constraint stating that classes that implement it must
            // have a public parameterless constructor
            object entityInstance = Activator.CreateInstance(configuringType);
            ...
            // the implemented IDbSeed<> OnModelConfiguring method. Using object as the type paraemeter
            // because the method name is indepenedent of the type parameter
            var seedMethod = configuringType.GetMethod(nameof(IDbSeed<object>.SeedData));

            // invoking the SeedData method, passing the EntityTypeBuilder parameter
            seedMethod.Invoke(entityInstance, new object[] { entityMethod.Invoke(builder, null) });

        },
        // The entity type can only configure itself. So if the generic parameter does not match
        // the type that implements the interface, we won't process it
        t => typeof(IDbSeed<>).MakeGenericType(t).IsAssignableFrom(t));
